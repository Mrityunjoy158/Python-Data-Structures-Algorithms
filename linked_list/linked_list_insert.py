...
class LinkedList:
    ...
    def insert(self, index, value):
        ...
        Handles three cases:
        1. Inserting at the beginning (index == 0): Uses `prepend()`.
        2. Inserting at the end (index == length): Uses `append()`.
        3. Inserting in the middle: Locates the node before the desired position and inserts the new node.

        Parameters:
        ----------
        index : int
            The position where the new node should be inserted. Must be in the range [0, length].
        ...
        Side Effects:
        ------------
        Increases the `length` of the linked list by 1 if insertion is successful.
        """
        if index < 0 or index > self.length:
            return False
        if index == 0:
